...
superMarioBros::FireBall::FireBall(diji::GameObject* ownerPtr, diji::Collider* owner, const bool isGoingRight)
    : Component{ ownerPtr }
...
void superMarioBros::FireBall::Init()
{
    ...
    collider->ApplyImpulse(sf::Vector2f{ m_IsGoingRight ? 850.f : -850.f , 850.f });
    collider->SetRestitution(1.2f);
    collider->SetMass(0.5f);
    collider->SetStaticFriction(0.0f);
    collider->SetKineticFriction(0.0f);
    collider->SetMaxVelocity(sf::Vector2f{ 850.f, 600.f });
...
void superMarioBros::FireBall::LateUpdate()
{
...
void superMarioBros::FireBall::OnDestroy()
{
    GameManager::GetInstance().FireballRemoved();
}

void superMarioBros::FireBall::OnHitEvent(const diji::Collider* other, const diji::CollisionInfo& hitInfo)
{
...
void superMarioBros::FireBall::OnTriggerEnter(const diji::Collider* other, const diji::CollisionInfo&)
{
...
void superMarioBros::FireBall::SpawnVFX() const
{
    Destroy();
