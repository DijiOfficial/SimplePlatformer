...
const std::vector<int> superMarioBros::MarioHelpers::s_StompPointsTable =
{
    100,   // 1st stomp
    200,   // 2nd stomp  
    400,   // 3rd stomp
...
bool superMarioBros::MarioHelpers::DoesPlayerHitBottomOfBlock(const sf::Vector2f& playerCenter, const sf::FloatRect& blockAABB, const sf::Vector2f& normal)
{
...
void superMarioBros::MarioHelpers::CheckForCollisionAboveBlock(const diji::Collider* collider)
{
    ...
    if (const auto hit =  diji::SceneManager::GetInstance().GetPhysicsWorld()->Raycast(TopLeft, dir, 15.f, collider))
    {
        if (hit->info.hasCollision && (hit->collider->GetTag() == "enemy" || hit->collider->GetTag() == "powerUp" || hit->collider->GetTag() == "coin"))
            diji::InterfaceRegistry::GetInterface<superMarioBros::IBumpable>(hit->collider->GetParent())->HandleBumpedBehavior(true);
...
std::string superMarioBros::MarioHelpers::GetStompPointsAsString(const int bounceMultiplier)
{
