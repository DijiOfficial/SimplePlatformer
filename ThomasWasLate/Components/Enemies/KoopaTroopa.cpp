...
void thomasWasLate::KoopaTroopa::OnHitEvent(const diji::Collider* other, const diji::CollisionInfo& hitInfo)
{
    if (m_Paused) return;
...
void thomasWasLate::KoopaTroopa::OnTriggerExit(const diji::Collider* other, const diji::CollisionInfo&)
{
...
void thomasWasLate::KoopaTroopa::OnTriggerEnter(const diji::Collider* other, const diji::CollisionInfo&)
{
    ...
    if (other->GetTag() == "enemy" || other->GetTag() == "koopa")
    {
        ...
        SpawnPointsText(mario::MarioHelpers::GetStompPointsAsString(m_CurrentComboIndex));
        ++m_CurrentComboIndex;
    }
}
...
void thomasWasLate::KoopaTroopa::SetShellAppearance() const
{
    m_SpriteRenderCompPtr->SetStartingFrame(4, 0);
    m_SpriteRenderCompPtr->SetTotalAnimationFrames(0);
    m_SpriteRenderCompPtr->SetFrameDuration(0.1f);
...
void thomasWasLate::KoopaTroopa::HandleStomp()
{
    SetRespawnTimer();
...
void thomasWasLate::KoopaTroopa::HandleStopBumpMovement()
{
    SetRespawnTimer();
    m_ColliderCompPtr->ClearAllOverlappedCollider();
    m_CurrentComboIndex = BASE_COMBO_INDEX;
